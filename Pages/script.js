const temp = document.getElementById("temp"),
  ...
  sunRise = document.querySelector(".sun-rise"), //  Sunrise
  // sunSet = document.querySelector(".sun-set"), // Sunset
  ...
  weatherCards = document.querySelector("#weather-cards"), // Weather Cards
...
function getWeatherData(city, unit, hourlyorWeek) {
  console.log(city);
  const apiKey = "97VYV9KFUFSQJFEY3LRAJ7YGU";
  fetch(
    `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${city}?unitGroup=metric&key=${apiKey}&contentType=json`,
    {
      method: "GET",
    ...
    .catch((err) => {
      alert("Kota tidak ditemukan di basis data kami");
    });
}
...
function updateVisibilityStatus(visibility) {
  if (visibility <= 0.3) {
    ...
    visibilityStatus.innerText = "Kabut Cahaya";
  } else if (visibility <= 1.13) {
    visibilityStatus.innerText = "Kabut";
  } else if (visibility <= 2.16) {
    visibilityStatus.innerText = "Kabut Ringan";
  } else if (visibility <= 5.4) {
    visibilityStatus.innerText = "Kabut Sangat Ringan";
...
function updateAirQualityStatus(airQuality) {
  if (airQuality <= 50) {
    airQualityStatus.innerText = "Bagus";
  } else if (airQuality <= 100) {
    airQualityStatus.innerText = "Sedang";
  } else if (airQuality <= 150) {
    airQualityStatus.innerText = "Tidak sehat untuk Kelompok Sensitif";
